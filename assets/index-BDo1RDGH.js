(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const i of t)if(i.type==="childList")for(const h of i.addedNodes)h.tagName==="LINK"&&h.rel==="modulepreload"&&n(h)}).observe(document,{childList:!0,subtree:!0});function a(t){const i={};return t.integrity&&(i.integrity=t.integrity),t.referrerPolicy&&(i.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?i.credentials="include":t.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(t){if(t.ep)return;t.ep=!0;const i=a(t);fetch(t.href,i)}})();function K(e,s){if(localStorage.getItem("weatherApiKey")){s();return}e.innerHTML=`
  ...
  `;const n=e.querySelector("#accept-waiver"),t=e.querySelector("#decline-waiver");n.addEventListener("click",()=>{localStorage.setItem("waiverAccepted","true"),e.innerHTML="",s()}),t.addEventListener("click",()=>{e.innerHTML="<p>You must accept the disclaimer to use PipeSafe.</p>"})}function j(e,s,a,n,t){let i=p(),h="not-safe";console.log("WeatherDisplay: Loaded settings:",i);function p(){const c=localStorage.getItem("pipeSafeSettings");return c?JSON.parse(c):{humidityThreshold:80,carTempIncrease:0}}const u=c=>{if(!Number.isFinite(c))return{status:"not-safe",reason:"Invalid data"};const d=Math.round(c);return d<50?{status:"not-safe",reason:`Too cold: ${d}°F`}:d>=85?{status:"not-safe",reason:`Too hot: ${d}°F`}:d>=83&&d<85?{status:"warning",reason:"Temperature near unsafe threshold"}:d>=50&&d<=52?{status:"warning",reason:"Temperature near unsafe threshold"}:{status:"safe",reason:"Conditions ideal"}},g=c=>Number.isFinite(c)?Math.round(c).toString().padStart(2,"0"):"N/A",w=c=>{if(!Number.isFinite(c))return"#888888";const d=Math.round(c);return`hsl(${200-(Math.min(Math.max(d,50),85)-50)/35*200}, 70%, 60%)`},b=c=>{if(!Number.isFinite(c))return"#888888";const d=Math.round(c),S=Math.min(Math.max(d,50),i.humidityThreshold),o=30+(S-50)/(i.humidityThreshold-50)*40,r=80-(S-50)/(i.humidityThreshold-50)*40;return`hsl(200, ${o}%, ${r}%)`},P=(c="Not Safe")=>{console.log("WeatherDisplay: Updating weather, scenario:",c),n.fetchWeatherData().then(([d])=>{if(console.log("WeatherDisplay: Current:",d),d.cod!==200){e.innerHTML="<p>Error: Invalid API response.</p>";return}const S=a.getParkingCondition();console.log("WeatherDisplay: Applying parking condition:",S);let o=i.carTempIncrease;S==="shade"&&(o-=5),console.log("WeatherDisplay: Temperature adjustment:",o);const r=Number.isFinite(d.main?.temp)?d.main.temp+o:NaN,m=Number.isFinite(d.main?.humidity)?d.main.humidity:NaN;console.log("WeatherDisplay: Current temp after adjustment:",r);const{status:f,reason:v}=u(r);h=f;const F=w(r),M=b(m),C=f==="safe"?"/pipe-safe/safe.svg":f==="warning"?"/pipe-safe/warning.svg":v==="Invalid data"?"/pipe-safe/error.svg":"/pipe-safe/not-safe.svg",E=f==="safe"||f==="warning";e.innerHTML=`
          
          <div class="current-weather">
            <div class="weather-item">
              <span class="label">Temperature</span>
              <span class="value" style="color: ${F};"><i class="fa-solid fa-temperature-quarter"></i> ${g(r)}°F</span>
            </div>
            <div class="weather-item">
              <span class="label">Humidity</span>
              <span class="value" style="color: ${M};"><i class="fa-solid fa-droplet"></i> ${g(m)}%</span>
            </div>
          </div>
          <img class="safety-image" src="${C}" alt="${f}">
          <p class="reason">Reason: ${v}</p>
        `,t.updateForecast(r,m,E,h)}).catch(d=>{console.error("WeatherDisplay: Fetch error:",d),e.innerHTML="<p>Error fetching weather data.</p>"})};return document.addEventListener("parking-changed",c=>{console.log("WeatherDisplay: Parking changed event received:",c.detail.parkingCondition),P(localStorage.getItem("testScenario")||"Not Safe")}),document.addEventListener("settings-changed",c=>{i=c.detail.settings,console.log("WeatherDisplay: Settings changed, new settings:",i),P(localStorage.getItem("testScenario")||"Not Safe")}),P(),{checkSafety:u,getTempColor:w,getHumidityColor:b,get currentStatus(){return h}}}function q(e){e.innerHTML=`
  ...
  `;const n=e.querySelector(".modal"),t=e.querySelector(".settings-button"),i=e.querySelector("#close-settings"),h=e.querySelector("#save-settings"),p=()=>{n.classList.remove("active")};t.addEventListener("click",()=>{n.classList.add("active")}),i.addEventListener("click",p),document.addEventListener("keydown",u=>{u.key==="Escape"&&n.classList.contains("active")&&p()}),n.addEventListener("click",u=>{u.target===n&&p()}),h.addEventListener("click",()=>{const u=parseInt(e.querySelector('input[name="humidity-threshold"]:checked')?.value||"80"),g=parseInt(e.querySelector('input[name="car-temp-increase"]:checked')?.value||"0"),w={humidityThreshold:u,carTempIncrease:g};console.log("SettingsModal: Saving settings:",w),localStorage.setItem("pipeSafeSettings",JSON.stringify(w));const b=new CustomEvent("settings-changed",{detail:{settings:w}});document.dispatchEvent(b),console.log("SettingsModal: Dispatched settings-changed"),p()})}function B(e){if(!e)throw console.error("ParkingToggle: Container not found!"),new Error("Parking toggle container not found");let a=localStorage.getItem("parkingStorageItem")||"open",n=t();function t(){const p=localStorage.getItem("pipeSafeSettings");return p?JSON.parse(p):{carTempIncrease:0}}const i=()=>n.carTempIncrease===0?a==="shade"?"Temperature Adjusted for Shade (-5°F)":"Cars heat up fast, temps may rise 10-20°F inside.":`Temperature Estimated for Car (${a==="shade"?`+${n.carTempIncrease-5}°F`:`+${n.carTempIncrease}°F`})`,h=()=>{const p=i();e.innerHTML=`
          ...
          <div class="forecast-well">
            ${I.map(l=>`
                <div class="forecast-box">
                  ...
                  <span class="temp" style="color: ${n(l.temp)};"><i class="fa-solid fa-temperature-quarter"></i> ${d(l.temp)}°F</span>
                  <span class="humidity" style="color: ${t(l.humidity)};"><i class="fa-solid fa-droplet"></i> ${d(l.humidity)}%</span>
                </div>
              `).join("")}
          </div>
        `}).catch(v=>{console.error("WeatherForecast: Fetch error:",v),e.innerHTML="<p>Error fetching forecast data.</p>"})};return document.addEventListener("parking-changed",o=>{console.log("WeatherForecast: Parking changed event received:",o.detail.parkingCondition),S(h,p,u,g)}),document.addEventListener("settings-changed",o=>{w=o.detail.settings,console.log("WeatherForecast: Settings changed, new settings:",w),S(h,p,u,g)}),{updateForecast:S}}"serviceWorker"in navigator&&navigator.serviceWorker.register("/pipe-safe/sw.js").then(e=>{console.log("Service Worker registered with scope:",e.scope)}).catch(e=>{console.error("Service Worker registration failed:",e)});const W=document.querySelector("#app"),A=()=>{const e=localStorage.getItem("weatherApiKey"),s=localStorage.getItem("waiverAccepted");if(console.log("main.ts: renderApp called, apiKey:",!!e,"waiverAccepted:",s),e&&s==="true"){W.innerHTML=`
      <div>
        <h1>PipeSafe</h1>
    ...
    `,H(document.querySelector("#settings-container")),q(document.querySelector("#info-container")),document.querySelector("#api-key-container").innerHTML="";const a=document.querySelector("#parking-toggle-container");let n;try{console.log("main.ts: Initializing parking toggle"),n=B(a)}catch(g){console.error("main.ts: Failed to initialize parking toggle:",g),W.innerHTML="<p>Error: Unable to initialize parking toggle.</p>";return}const t=z(),i=document.querySelector("#weather-container"),h=document.querySelector("#weather-forecast-container");console.log("main.ts: Initializing weather forecast");const p=x(h,t,g=>({status:"not-safe",reason:"Placeholder"}),g=>"#888888",g=>"#888888",n);console.log("main.ts: Initializing weather display");const u=j(i,e,n,t,p);console.log("main.ts: Updating weather forecast with display functions"),x(h,t,u.checkSafety,u.getTempColor,u.getHumidityColor,n)}else W.innerHTML=`
      <div>
        <h1>PipeSafe</h1>
        <div id='settings-container'></div>
        <div id='info-container'></div>
