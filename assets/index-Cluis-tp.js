(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))o(t);new MutationObserver(t=>{for(const i of t)if(i.type==="childList")for(const m of i.addedNodes)m.tagName==="LINK"&&m.rel==="modulepreload"&&o(m)}).observe(document,{childList:!0,subtree:!0});function a(t){const i={};return t.integrity&&(i.integrity=t.integrity),t.referrerPolicy&&(i.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?i.credentials="include":t.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(t){if(t.ep)return;t.ep=!0;const i=a(t);fetch(t.href,i)}})();function _(e,s){if(localStorage.getItem("weatherApiKey")){s();return}e.innerHTML=`
  ...
  `;const o=e.querySelector("#save-api-key"),t=e.querySelector("#api-key-input");o.addEventListener("click",()=>{const i=t.value.trim();i?(localStorage.setItem("weatherApiKey",i),e.innerHTML="",s()):(t.placeholder="API Key cannot be empty",t.value="")})}function K(e,s){if(localStorage.getItem("waiverAccepted")==="true"){s();return}e.innerHTML=`
  ...
  `;const o=e.querySelector("#accept-waiver"),t=e.querySelector("#decline-waiver");o.addEventListener("click",()=>{localStorage.setItem("waiverAccepted","true"),e.innerHTML="",s()}),t.addEventListener("click",()=>{e.innerHTML="<p>You must accept the disclaimer to use PipeSafe.</p>"})}function O(e,s,a,o,t){let i=p(),m="not-safe";console.log("WeatherDisplay: Loaded settings:",i);function p(){const d=localStorage.getItem("pipeSafeSettings");return d?JSON.parse(d):{humidityThreshold:80,carTempIncrease:0}}const u=(d,h)=>{if(!Number.isFinite(d)||!Number.isFinite(h))return{status:"not-safe",reason:"Invalid data"};const n=Math.round(d),r=Math.round(h),c=i.humidityThreshold;return n<50?{status:"not-safe",reason:`Too cold: ${n}°F`}:n>=85?{status:"not-safe",reason:`Too hot: ${n}°F`}:r<50?{status:"not-safe",reason:`Too dry: ${r}%`}:r>c?{status:"not-safe",reason:`Too humid: ${r}%`}:n>=83&&n<85||n>=50&&n<=52||r>=c-2&&r<=c||r>=50&&r<=52?{status:"warning",reason:"Conditions near unsafe thresholds"}:{status:"safe",reason:"Conditions ideal"}},g=d=>Number.isFinite(d)?Math.round(d).toString().padStart(2,"0"):"N/A",f=d=>{if(!Number.isFinite(d))return"#888888";const h=Math.round(d);return`hsl(${200-(Math.min(Math.max(h,50),85)-50)/35*200}, 70%, 60%)`},w=d=>{if(!Number.isFinite(d))return"#888888";const h=Math.round(d),n=Math.min(Math.max(h,50),i.humidityThreshold),r=30+(n-50)/(i.humidityThreshold-50)*40,c=80-(n-50)/(i.humidityThreshold-50)*40;return`hsl(200, ${r}%, ${c}%)`},D=(d="Not Safe")=>{console.log("WeatherDisplay: Updating weather, scenario:",d),o.fetchWeatherData().then(([h])=>{if(console.log("WeatherDisplay: Current:",h),h.cod!==200){e.innerHTML="<p>Error: Invalid API response.</p>";return}const n=a.getParkingCondition();console.log("WeatherDisplay: Applying parking condition:",n);let r=i.carTempIncrease;n==="shade"&&(r-=5),console.log("WeatherDisplay: Temperature adjustment:",r);const c=Number.isFinite(h.main?.temp)?h.main.temp+r:NaN,v=Number.isFinite(h.main?.humidity)?h.main.humidity:NaN;console.log("WeatherDisplay: Current temp after adjustment:",c);const{status:k,reason:N}=u(c,v);m=k;const F=f(c),T=w(v),$=k==="safe"?"/pipe-safe/safe.svg":k==="warning"?"/pipe-safe/warning.svg":N==="Invalid data"?"/pipe-safe/error.svg":"/pipe-safe/not-safe.svg",L=k==="safe"||k==="warning";e.innerHTML=`
          ...
          <img class="safety-image" src="${$}" alt="${k}">
  ...
  `;const s=e.querySelector(".modal"),a=e.querySelector(".info-button"),o=e.querySelector("#close-info"),t=()=>{s.classList.remove("active")};a.addEventListener("click",()=>{s.classList.add("active")}),o.addEventListener("click",t),document.addEventListener("keydown",i=>{i.key==="Escape"&&s.classList.contains("active")&&t()}),s.addEventListener("click",i=>{i.target===s&&t()})}function H(e){const s=localStorage.getItem("pipeSafeSettings"),a=s?JSON.parse(s):{humidityThreshold:80,carTempIncrease:0};console.log("SettingsModal: Loaded settings:",a),e.innerHTML=`
  ...
  `;const o=e.querySelector(".modal"),t=e.querySelector(".settings-button"),i=e.querySelector("#close-settings"),m=e.querySelector("#save-settings"),p=()=>{o.classList.remove("active")};t.addEventListener("click",()=>{o.classList.add("active")}),i.addEventListener("click",p),document.addEventListener("keydown",u=>{u.key==="Escape"&&o.classList.contains("active")&&p()}),o.addEventListener("click",u=>{u.target===o&&p()}),m.addEventListener("click",()=>{const u=parseInt(e.querySelector('input[name="humidity-threshold"]:checked')?.value||"80"),g=parseInt(e.querySelector('input[name="car-temp-increase"]:checked')?.value||"0"),f={humidityThreshold:u,carTempIncrease:g};console.log("SettingsModal: Saving settings:",f),localStorage.setItem("pipeSafeSettings",JSON.stringify(f));const w=new CustomEvent("settings-changed",{detail:{settings:f}});document.dispatchEvent(w),console.log("SettingsModal: Dispatched settings-changed"),p()})}function j(e){if(!e)throw console.error("ParkingToggle: Container not found!"),new Error("Parking toggle container not found");let a=localStorage.getItem("parkingStorageItem")||"open",o=t();function t(){const p=localStorage.getItem("pipeSafeSettings");return p?JSON.parse(p):{carTempIncrease:0}}const i=()=>o.carTempIncrease===0?a==="shade"?"Temperature Adjusted for Shade (-5°F)":"Cars heat up fast, temps may rise 10-20°F inside.":`Temperature Estimated for Car (${a==="shade"?`+${o.carTempIncrease-5}°F`:`+${o.carTempIncrease}°F`})`,m=()=>{const p=i();e.innerHTML=`
    ...
    `;const u=e.querySelector("#parking-toggle"),g=e.querySelector(".parking-toggle-container");g&&u?g.addEventListener("click",f=>{f.preventDefault(),u.checked=!u.checked,a=u.checked?"open":"shade",console.log("ParkingToggle: Changed to:",a),localStorage.setItem("parkingStorageItem",a),m();const w=new CustomEvent("parking-changed",{detail:{parkingCondition:a}});console.log("ParkingToggle: Dispatching parking-changed event:",w),document.dispatchEvent(w)}):console.error("ParkingToggle: Toggle input or container not found!")};return document.addEventListener("settings-changed",p=>{o=p.detail.settings,console.log("ParkingToggle: Settings changed, new settings:",o),m()}),m(),{getParkingCondition:()=>(console.log("ParkingToggle: getParkingCondition called, returning:",a),a)}}const B={Safe:{current:{cod:200,main:{temp:65,humidity:50}},forecast:{cod:"200",list:[{dt:Math.floor(Date.now()/1e3)+3600,main:{temp:76,humidity:69},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+7200,main:{temp:77,humidity:68},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+10800,main:{temp:78,humidity:67},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+14400,main:{temp:84,humidity:78},dt_txt:""}]}},Warning:{current:{cod:200,main:{temp:83,humidity:78}},forecast:{cod:"200",list:[{dt:Math.floor(Date.now()/1e3)+3600,main:{temp:84,humidity:79},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+7200,main:{temp:85,humidity:80},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+10800,main:{temp:75,humidity:70},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+14400,main:{temp:74,humidity:69},dt_txt:""}]}},"Not Safe":{current:{cod:200,main:{temp:77,humidity:86}},forecast:{cod:"200",list:[{dt:Math.floor(Date.now()/1e3)+3600,main:{temp:78,humidity:85},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+7200,main:{temp:79,humidity:84},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+10800,main:{temp:80,humidity:83},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+14400,main:{temp:75,humidity:70},dt_txt:""}]}},"All Safe":{current:{cod:200,main:{temp:70,humidity:65}},forecast:{cod:"200",list:[{dt:Math.floor(Date.now()/1e3)+3600,main:{temp:71,humidity:64},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+7200,main:{temp:72,humidity:63},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+10800,main:{temp:73,humidity:62},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+14400,main:{temp:74,humidity:61},dt_txt:""}]}},"All Not Safe":{current:{cod:200,main:{temp:90,humidity:90}},forecast:{cod:"200",list:[{dt:Math.floor(Date.now()/1e3)+3600,main:{temp:91,humidity:89},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+7200,main:{temp:92,humidity:88},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+10800,main:{temp:93,humidity:87},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+14400,main:{temp:94,humidity:86},dt_txt:""}]}},"Invalid Data":{current:{cod:200,main:{temp:NaN,humidity:NaN}},forecast:{cod:"200",list:[{dt:Math.floor(Date.now()/1e3)+3600,main:{temp:NaN,humidity:NaN},dt_txt:""},{dt:Math.floor(Date.now()/1e3)+7200,main:{temp:75,humidity:70},dt_txt:""}]}}};function z(e,s){return{fetchWeatherData:async()=>{{const o=localStorage.getItem("testScenario")||"Not Safe";console.log("Fetching test scenario:",o);const t=B[o];return t?Promise.resolve([t.current,t.forecast]):(console.error("Invalid test scenario:",o),Promise.reject(new Error("Invalid test scenario")))}}}}function q(e,s,a,o,t,i){let m=0,p=0,u=!1,g="not-safe",f=w();function w(){const n=localStorage.getItem("pipeSafeSettings");return n?JSON.parse(n):{humidityThreshold:80,carTempIncrease:0}}const D=(n,r,c)=>{if(!Number.isFinite(r.temp)||!Number.isFinite(c.temp)||!Number.isFinite(r.humidity)||!Number.isFinite(c.humidity)||r.dt>=c.dt)return{time:new Date(n*1e3).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),temp:r.temp||c.temp||0,humidity:r.humidity||c.humidity||0};const v=(n-r.dt)/(c.dt-r.dt);return{time:new Date(n*1e3).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),temp:Math.round(r.temp+(c.temp-r.temp)*v),humidity:Math.round(r.humidity+(c.humidity-r.humidity)*v)}},d=n=>Number.isFinite(n)?Math.round(n).toString().padStart(2,"0"):"N/A",h=(n,r,c,v)=>{m=n,p=r,u=c,g=v,console.log("WeatherForecast: Updating forecast, currentTemp:",n),s.fetchWeatherData().then(([k,N])=>{console.log("WeatherForecast: Forecast fetched:",N),console.log("WeatherForecast: Loaded settings:",f);const F=i.getParkingCondition();console.log("WeatherForecast: Applying parking condition:",F);let T=f.carTempIncrease;F==="shade"&&(T-=5),console.log("WeatherForecast: Temperature adjustment:",T);const L=Math.floor(new Date().getTime()/1e3),A=L+12*3600,M=[{time:"Now</br></br>",temp:n,humidity:r}],I=(N.list||[]).filter(l=>l.dt>=L&&l.dt<=A+3600&&Number.isFinite(l.main?.temp)&&Number.isFinite(l.main?.humidity)).slice(0,7);if(I.length<1){e.innerHTML="<p>Error: No valid forecast data.</p>";return}for(let l=1;l<=12;l++){const S=L+l*3600,y=I.slice().reverse().find(b=>b.dt<=S)||I[0],P=I.find(b=>b.dt>=S)||I[I.length-1];if(!y||!P){M.push({time:new Date(S*1e3).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),temp:n,humidity:r});continue}if(y.dt===S||Math.abs(y.dt-S)<300)M.push({time:new Date(y.dt*1e3).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),temp:y.main.temp+T,humidity:y.main.humidity});else{const b=D(S,{dt:y.dt,temp:y.main.temp+T,humidity:y.main.humidity},{dt:P.dt,temp:P.main.temp+T,humidity:P.main.humidity});M.push(b)}}let C="";if(M.slice(1).every(l=>a(l.temp,l.humidity).status===(c?v:"not-safe")))C=c?"Safe all day!":"Not safe all day!";else for(let l=1;l<M.length;l++){const S=M[l],y=a(S.temp,S.humidity).status;if((y==="safe"||y==="warning")!==c){const b=new Date(L*1e3+l*3600);C=c?`Safe until ${b.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})}`:`Not safe until ${b.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})}`;break}}e.innerHTML=`
        ...
        `}).catch(k=>{console.error("WeatherForecast: Fetch error:",k),e.innerHTML="<p>Error fetching forecast data.</p>"})};return document.addEventListener("parking-changed",n=>{console.log("WeatherForecast: Parking changed event received:",n.detail.parkingCondition),h(m,p,u,g)}),document.addEventListener("settings-changed",n=>{f=n.detail.settings,console.log("WeatherForecast: Settings changed, new settings:",f),h(m,p,u,g)}),{updateForecast:h}}"serviceWorker"in navigator&&navigator.serviceWorker.register("/pipe-safe/sw.js").then(e=>{console.log("Service Worker registered with scope:",e.scope)}).catch(e=>{console.error("Service Worker registration failed:",e)});const W=document.querySelector("#app"),x=()=>{const e=localStorage.getItem("weatherApiKey"),s=localStorage.getItem("waiverAccepted");if(console.log("main.ts: renderApp called, apiKey:",!!e,"waiverAccepted:",s),e&&s==="true"){W.innerHTML=`
      <div>
        <h1>PipeSafe</h1>
    ...
    `,H(document.querySelector("#settings-container")),E(document.querySelector("#info-container")),document.querySelector("#api-key-container").innerHTML="";const a=document.querySelector("#parking-toggle-container");let o;try{console.log("main.ts: Initializing parking toggle"),o=j(a)}catch(g){console.error("main.ts: Failed to initialize parking toggle:",g),W.innerHTML="<p>Error: Unable to initialize parking toggle.</p>";return}const t=z(),i=document.querySelector("#weather-container"),m=document.querySelector("#weather-forecast-container");console.log("main.ts: Initializing weather forecast");const p=q(m,t,(g,f)=>({status:"not-safe",reason:"Placeholder"}),g=>"#888888",g=>"#888888",o);console.log("main.ts: Initializing weather display");const u=O(i,e,o,t,p);console.log("main.ts: Updating weather forecast with display functions"),q(m,t,u.checkSafety,u.getTempColor,u.getHumidityColor,o)}else W.innerHTML=`
      <div>
        <h1>PipeSafe</h1>
        <div id='settings-container'></div>
        <div id='info-container'></div>
    ...
    `,H(document.querySelector("#settings-container")),E(document.querySelector("#info-container")),e?document.querySelector("#api-key-container").innerHTML="":_(document.querySelector("#api-key-container"),x)};K(W,x);
