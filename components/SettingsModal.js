...
export function setupSettingsModal(container) {
    // Load settings
  ...
  `;
    ...
    saveButton.addEventListener('click', () => {
        const humidityThreshold = parseInt(container.querySelector('input[name="humidity-threshold"]:checked')?.value || '80');
        const carTempIncrease = parseInt(container.querySelector('input[name="car-temp-increase"]:checked')?.value || '0');
        const newSettings = {
            humidityThreshold,
            carTempIncrease,
        };
